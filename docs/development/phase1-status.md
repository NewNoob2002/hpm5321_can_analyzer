...
| Protocol v1 device session core | PASS | Pure-C99 session engine under `protocol/v1/c` implements CAS generations, arm epoch, conservative TX admission (token buckets + reserved load), exactly-once TX result ledger, replay cache, bounded-loss accounting and priority egress queues; 60+ host scenario assertions pass (ASan/UBSan + RISC-V cross compile) |
| Host transport + fake backend | PASS | `host/crates/core` (hpm-usb-can-core) defines the frame-oriented `Transport` trait with `FakeTransport`/`FakeDevice` (in-memory v1.0 device: negotiation, CAS, arm epoch, TX ledger, replay cache, capture streaming, loss injection, hotplug reset) and a rusb `UsbTransport`; 15 fake-backend parity flows pass, clippy clean |
