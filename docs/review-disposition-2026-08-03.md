...
| External evidence not reconstructible from revision | Yes | File-level source manifest and dependency lock added for the new safe-order images; changes still must be committed before a clean Git checkout can reproduce them; historical TX evidence remains non-formal |
...
| Claimed source-bound TX ELF predates current manifest | Yes | Corrected: `b1076d8e...` is superseded. Current ABI-v4 ELF is `bbbef9d7...`, source manifest `332d2a05...`, SDK commit `88b01b43...`; a dedicated artifact validator now checks the attestation, but target/external validation remains open |
| Fixes absent from version control | Yes | Still open: index is empty and major additions remain untracked. Documentation now says present/untracked rather than staged; clean-checkout Gate remains blocked |
...
| Result ABI changed but version stayed 1 | Yes | Fixed: current external-probe result ABI is version 4; historical v1/v2 evidence remains labeled historical |
...
| Python cache files pollute workspace | Yes | Added ignore rules and removed tracked/untracked `__pycache__` artifacts from the worktree; deletion still needs committing |
...
| Timestamp negative test exits at capture hash | Yes | Test now recomputes sidecar capture hash and asserts the specific timestamp-monotonicity failure; current suite is 11/11 |
| Cleanup snapshot precedes cleanup | Yes | ABI v4 waits for INIT and gates DONE on post-cleanup CCCR, PB00/PB01 IOC FUNC_CTL, GPIOB OE and GPIOM readback; current ELF still requires fresh target evidence |
...
| `.omx` and Python cache workspace state regressed | Yes | `.omx/` ignore restored; cache directories removed; tracked pyc files are worktree deletions awaiting commit |
