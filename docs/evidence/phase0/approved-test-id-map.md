...
| One-shot arm consumption and post-TX disconnect | T-CAN-013 subcase (target PASS, external capture pending) | ABI-v4 proved token consumption, 100/100 target TX and synchronous cleanup; the probe has no arm expiry, USB session reset/disconnect, immediate TX queue, or scheduled-TX queue, so formal T-CAN-013 remains NOT_TESTED |
