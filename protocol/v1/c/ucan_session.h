 ...
 * or 1 when the ring dropped the frame (accumulates DATA_LOSS, CHANNEL
 * domain). */
...
/* Host-request entry: builds the response frame (synchronous) and enqueues
 * async events. Returns 0 and fills out/out_len on success, or a negative
 * codec/session error. */
