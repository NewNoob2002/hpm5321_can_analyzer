...
void usbd_winusb_out(uint8_t busid, uint8_t ep, uint32_t nbytes)
{
    ...
    /*
     * Keep ownership of read_buffer until the matching IN transfer completes.
     * The upstream demo re-armed OUT immediately and could overwrite the echo
     * payload while DMA was still consuming it.
     */
    if (usbd_ep_start_write(busid, WINUSB_IN_EP, read_buffer, nbytes) < 0) {
        usbd_ep_start_read(busid, ep, read_buffer, sizeof(read_buffer));
    }
}
...
void usbd_winusb_in(uint8_t busid, uint8_t ep, uint32_t nbytes)
{
    ...
    /*
     * The host requests the exact echoed length, so an additional ZLP would
     * become the next transaction's unexpected zero-byte response.
     */
    ep_tx_busy_flag = false;
