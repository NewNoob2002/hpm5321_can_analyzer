...
/* When USE_SYSCALL_INTERRUPT_PRIORITY is set, interrupts whose priority is
   higher than configMAX_SYSCALL_INTERRUPT_PRIORITY will not be delayed by
   anything FreeRTOS do. */
 ...
 * be at least big enough to contain the output of the task-stats command, as
 * the example implementation does not include buffer overlow checking.
 */
 ...
 * allow the application writer to add additional code before and after the MCU
 * is placed into the low power state respectively.  The empty implementations
